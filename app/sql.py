...
def sql_chain(questions):
    sql_query = generate_sql_query(question=questions)
    pattern = pattern = r"<SQL>(.*?)</SQL>"
    matches = re.findall(pattern, sql_query, re.DOTALL | re.IGNORECASE)
    if len(matches) == 0:
        return "Sorry LLM did not generate query for your questions"

    response = run_query(matches[0].strip())
    if response is None:
        return "Sorry there was error in executing SQL query"

    context = response.to_dict(orient='records')
    answer = data_comprehension(questions,context)
    return answer
...
if __name__ == '__main__':
    question = 'show me puma shoes with discount higher than 30% and avg_rating higher than 4.7'
    query = generate_sql_query(question=question)
    print(query)